...
with st.sidebar:
    ...
    st.divider()
    st.markdown("### 📁 Model Paths")
    text_model_standard = st.text_input("Standard model (.h5)", value="lstm_main.h5")
    text_tok_standard = st.text_input("Standard tokenizer (.pkl)", value="tokenizer.pkl")
    text_model_pro = st.text_input("Pro model (.h5)", value="lstm_kfold.h5")
    text_tok_pro = st.text_input("Pro tokenizer (.pkl)", value="tokenizer_lstm_best_kfold.pkl")
    st.divider()
    openai_api_key = st.text_input("OpenAI API Key (for Humanizer)", type="password", value=os.environ.get("OPENAI_API_KEY", ""))
...
# Keep your existing functions: text_predict, text_verdict, _openai_call, humanize

# UI (I kept your original logic with small fixes)
st.markdown('<div class="hero-title">📝 AI Text Detector</div>', unsafe_allow_html=True)
st.caption("Detect AI-generated text + GPT-4o Humanizer")

# ... (Rest of your Text UI code remains almost same. Let me know if you want full 400+ lines version)
