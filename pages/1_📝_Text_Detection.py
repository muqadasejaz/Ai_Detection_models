...
st.set_page_config(
    page_title="Text Detection · DeepSentinel",
    page_icon="📝",
    layout="wide"
)
...
with st.sidebar:
    ...
    st.divider()
    st.markdown("### 📁 Model Paths")
    text_model_standard = st.text_input(
        "Standard model (.h5)", 
        value=os.environ.get("TEXT_MODEL_STANDARD", "lstm_main.h5")
    )
    text_tok_standard = st.text_input(
        "Standard tokenizer (.pkl)", 
        value=os.environ.get("TEXT_TOK_STANDARD", "tokenizer.pkl")
    )
    text_model_pro = st.text_input(
        "Pro model (.h5)", 
        value=os.environ.get("TEXT_MODEL_PRO", "lstm_kfold.h5")
    )
    text_tok_pro = st.text_input(
        "Pro tokenizer (.pkl)", 
        value=os.environ.get("TEXT_TOK_PRO", "tokenizer_lstm_best_kfold.pkl")
    )
    st.divider()
    st.markdown("### 🔑 OpenAI API Key")
...
TEXT_MODELS = {
    "⚡ Standard": {
        "model_path": text_model_standard,
        "tokenizer_path": text_tok_standard,
        "description": "Single-pass training. Fast and reliable."
    },
    "🏆 Pro": {
        "model_path": text_model_pro,
        "tokenizer_path": text_tok_pro,
        "description": "5-fold cross-validation. Better on difficult texts."
    },
}
...
H_PROMPTS = [
    """You are a professional human writer. Rewrite the AI-generated text so no detector flags it,
keeping every fact and meaning intact. Vary sentence length aggressively. Use contractions everywhere.
Replace formal words with conversational ones. Delete filler phrases. Let paragraphs be uneven.
Occasionally start sentences with But/And/So.
OUTPUT: only the rewritten text.""",
    """You are rewriting text an AI detector still flagged. Go further: restructure sentences completely,
split long sentences, add natural imperfections. Keep all facts.
OUTPUT: only the rewritten text.""",
    """CRITICAL REWRITE — flagged twice. Rewrite from scratch in your own voice while keeping every fact.
Vary paragraph length heavily. Contractions throughout. Avoid repetitive starters.
FORBIDDEN: Furthermore, Moreover, In addition, Leverage, Utilize, Delve into.
OUTPUT: only the rewritten text."""
]
...
def text_verdict(prob):
    ...
    hu = (1 - prob) * 100
    if prob > 0.75:
        v, c = "🤖 Likely AI-Generated", "#e74c3c"
    elif prob > 0.5:
        v, c = "🤖 Possibly AI-Generated", "#e67e22"
    elif prob > 0.25:
        v, c = "✍️ Possibly Human-Written", "#2980b9"
    else:
        v, c = "✍️ Likely Human-Written", "#27ae60"
    return v, c, ai, hu
...
def humanize(text, api_key, model, tokenizer, threshold=0.45, aggressiveness="Balanced", status_cb=None):
    ...
    for i in range(s_idx, s_idx + mx):
        ...
        pn = i - s_idx + 1
        if status_cb:
            status_cb(f"Pass {pn}: {['light','structural','deep'][idx]} rewrite…")
...
if "ths" not in st.session_state:
    st.session_state.ths = None
if "tpr" not in st.session_state:
    st.session_state.tpr = []
...
sel = st.radio("Detection Model", list(TEXT_MODELS.keys()), horizontal=True, index=0)
mi = TEXT_MODELS[sel]
st.caption(f"ℹ️ {mi['description']}")

st.divider()

tin = st.text_area("Paste your text here", height=220,
                   placeholder="Type or paste text to analyze…", max_chars=5000)
...
if st.button("🔍 Analyze Text", type="primary", use_container_width=True):
    if not tin.strip():
        ...
        st.warning("Please paste at least 10 words for reliable detection.")
    else:
        st.session_state.th = None
        st.session_state.ths = None
        st.session_state.tpr = []
        ...
        with st.spinner("Analyzing..."):
            try:
                ...
                st.error(f"Model file not found: `{mi['model_path']}`. Check sidebar paths or run from main page.")
                st.stop()
            except Exception as e:
...
if st.session_state.tp is not None and st.session_state.tl == tin:
    ...
    if prob > 0.5:
        st.divider()
        st.subheader("✍️ Humanize This Text")
        st.caption("Multi-pass GPT-4o rewrite with automatic re-scoring")
        ...
        with cb:
            st.markdown("<br>", unsafe_allow_html=True)
            hbtn = st.button("✨ Humanize Text", type="primary", use_container_width=True,
                             disabled=not openai_api_key)

        if not openai_api_key:
            st.warning("Enter your OpenAI API key in the sidebar to enable humanizer.")
        ...
        if hbtn and openai_api_key:
            ...
            try:
                ...
                sb.empty()
                st.error(f"Humanizer failed: {e}")

    # Show Humanized Result
    if st.session_state.th:
        ht = st.session_state.th
        hs = st.session_state.ths
        pr = st.session_state.tpr
        _, _, nai, _ = text_verdict(hs)
        ...
        c1, c2, c3 = st.columns(3)
        c1.metric("Original AI Score", f"{ai_pct:.1f}%")
        c2.metric("Humanized AI Score", f"{nai:.1f}%", 
                  delta=f"{nai - ai_pct:+.1f}%", delta_color="inverse")
        c3.metric("Improvement", f"{ai_pct - nai:.1f}%")

        if len(pr) > 1:
            with st.expander(f"🔁 Rewrite Pipeline ({len(pr)} passes)"):
                for r in pr:
                    st.write(f"{'✅' if r['score'] <= 0.45 else '⚠️'} Pass {r['pass']}: **{r['score']*100:.1f}%**")

        st.text_area("Humanized Text", value=ht, height=280)

        with st.expander("📋 Side-by-Side Comparison"):
            l, r = st.columns(2)
            l.markdown("**Original**")
            l.markdown(f"<div style='background:#0e0f1a;padding:12px;border-radius:8px;'>{tin.replace(chr(10), '<br>')}</div>", unsafe_allow_html=True)
            r.markdown("**Humanized**")
            r.markdown(f"<div style='background:#0e0f1a;padding:12px;border-radius:8px;'>{ht.replace(chr(10), '<br>')}</div>", unsafe_allow_html=True)

st.markdown("""
<div class="info-box">
<b>Models:</b> Bidirectional LSTM (Standard) • 5-fold CV LSTM (Pro)<br>
<b>Humanizer:</b> GPT-4o • Up to 3 escalating passes with automatic re-scoring
</div>
