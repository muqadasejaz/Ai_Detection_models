...
#     st.markdown("### 📁 Model Directory")
#     vid_model_dir = st.text_input("Model directory", value=os.environ.get("VID_MODEL_DIR", "."),
#                                   help="Directory where .pt files are saved (auto-downloaded if missing)")
...
#         backbone   = tv_models.resnext50_32x4d(weights=None)
#         self.model = nn.Sequential(*list(backbone.children())[:-2])
#         # FIX: the 4th positional arg of nn.LSTM is `bias`, not `bidirectional`.
#         # Passing it positionally silently set bias=False. Use the keyword.
...
#     # Prefer an existing file in the user-specified dir; otherwise download once.
#     local_path = os.path.join(model_dir, model_filename)
#     if os.path.exists(local_path):
#         model_path = local_path
#     else:
#         model_path = ensure_file(model_filename)   # downloads once on first use
...
#     total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
#     fps          = cap.get(cv2.CAP_PROP_FPS) or 25.0
#     n            = max(1, min(sequence_length, total_frames))
#     indices      = [int(i * total_frames / n) for i in range(n)]
#     tensors, display_frames = [], []
#     for idx in indices:
#         cap.set(cv2.CAP_PROP_POS_FRAMES, idx)
