...
class _VideoModel(nn.Module):
    def __init__(self, num_classes=2, latent_dim=2048, lstm_layers=1, hidden_dim=2048):
        ...
        self.lstm = nn.LSTM(latent_dim, hidden_dim, lstm_layers, batch_first=True, bidirectional=False)
        self.dp = nn.Dropout(0.4)
...
VID_MODELS_AVAILABLE = {
    "97% acc · 100 frames": ("model_97_acc_100_frames_FF_data.pt", 100),
    "97% acc · 80 frames": ("model_97_acc_80_frames_FF_data.pt", 80),
    "97% acc · 60 frames": ("model_97_acc_60_frames_FF_data.pt", 60),
    "95% acc · 40 frames": ("model_95_acc_40_frames_FF_data.pt", 40),
    "93% acc · 100 frames": ("model_93_acc_100_frames_celeb_FF_data.pt", 100),
}

@st.cache_resource(show_spinner="Loading video model...")
def load_vid_model(model_filename, model_dir):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model_path = os.path.join(model_dir, model_filename)
    if not os.path.exists(model_path):
...
def _detect_and_crop_face(frame_bgr):
    ...
    if len(faces) > 0:
        ...
        return frame_bgr[max(0, y-pad):y+h+pad, max(0, x-pad):x+w+pad]
    return frame_bgr

# Keep your other helper functions (_extract_vid_frames, _get_vid_info, _run_vid_prediction)

# ==================== UI ====================
st.markdown('<div class="hero-title">🎬 Video Deepfake Detector</div>', unsafe_allow_html=True)
st.caption("ResNeXt50 + LSTM • Face-aware • Multiple pretrained models")

vl, vr = st.columns([1, 1], gap="large")

with vl:
    uvid = st.file_uploader("Upload video", type=["mp4","avi","mov","mkv","webm"])
    vid_model_choice = st.selectbox("Select Model", list(VID_MODELS_AVAILABLE.keys()), index=0)
    vid_model_filename, vid_seq_len = VID_MODELS_AVAILABLE[vid_model_choice]
    vid_run_all = st.checkbox("Run Ensemble (All 5 Models)", value=False)
    analyze_btn = st.button("🎬 Analyze Video", type="primary", use_container_width=True)

if uvid and not analyze_btn:
    with vr: st.video(uvid)

if uvid and analyze_btn:
    # ... (keep your original processing logic with the fixed result_card part)

    # At the end when showing result:
    device_str = str(device_v).upper() if 'device_v' in locals() else "CPU"
    ...
    with vr:
        vid_result_ph.markdown(
            result_card_html(
                verdict, css_cls, conf_pct,
                avg_fake if is_fake else avg_real,
                f"<b>Model</b> · {vid_model_choice}<br>"
                f"<b>Device</b> · {device_str} &nbsp;·&nbsp; <b>Frames</b> · {vid_seq_len}"
            ), unsafe_allow_html=True
        )
