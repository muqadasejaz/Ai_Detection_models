...
with st.sidebar:
    ...
    st.caption("AI & Deepfake Detection Suite")
    st.divider()
    audio_model_path = st.text_input("Audio Model Path", value="deepfake_audio_detector.h5")
...
def load_audio_model(path):
    ...
    if not os.path.exists(path):
        st.error(f"Audio model not found: {path}")
        st.stop()
...
def audio_features(fp):
    ...
    if mdb.shape[1] < AUD_FRAMES:
        mdb = np.pad(mdb, ((0,0), (0, AUD_FRAMES - mdb.shape[1])), mode='reflect')
    else:
        mdb = mdb[:, :AUD_FRAMES]
    return mdb[np.newaxis, ...], mdb, sr, dur

st.markdown('<div class="hero-title">🎙️ Audio Deepfake Detector</div>', unsafe_allow_html=True)
st.caption("Mel Spectrogram CNN • Supports WAV, MP3, FLAC, OGG")

sw = st.toggle("Show Waveform", value=True)
sm = st.toggle("Show Mel Spectrogram", value=True)

uaud = st.file_uploader("Upload audio", type=["wav","mp3","flac","ogg"])

if uaud:
    ab = uaud.read()
    if len(ab) / (1024**2) > AUD_MAX_MB:
        st.error(f"File too large. Max {AUD_MAX_MB} MB.")
        st.stop()

    st.audio(uaud)
    suf = "." + uaud.name.split(".")[-1]
    ...
    try:
        ...
        with st.spinner("Extracting features..."):
            feats, mdb, sr, dur = audio_features(tmp_path)

        with st.spinner("Running inference..."):
            pred = float(model.predict(feats, verbose=0)[0][0])
        ...
        rc, bc = st.columns([1, 2])
        with rc:
            st.markdown(result_card_html(lbl_a, cls_a, int(conf_a*100), pred, f"File: {uaud.name}"), unsafe_allow_html=True)
        ...
        if sw or sm:
            ...
            if sw:
                st.write("**Waveform**")
                # Add your plot_wave function here
            if sm:
                st.write("**Mel Spectrogram**")
                # Add your plot_mel function here
