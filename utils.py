...
[data-testid="stSidebar"] h1, [data-testid="stSidebar"] h2, [data-testid="stSidebar"] h3 { color: #d0d1e8 !important; }

.result-card { border-radius: 14px; padding: 1.6rem 1.8rem 1.4rem; border: 1px solid #1c1d30; background: #0e0f1a; position: relative; overflow: hidden; margin-bottom: 1rem; }
.result-card::before { content:''; position:absolute; top:0; left:0; right:0; height:3px; }
.result-card.fake::before, .result-card.ai::before { background: linear-gradient(90deg,#ff3a5c,#ff8c00); }
.result-card.real::before, .result-card.human::before { background: linear-gradient(90deg,#00d4aa,#0090ff); }
...
def download_models():
    ...
    bar.progress(1.0, text="✅ All models downloaded!")
    bar.empty()
    st.session_state.models_downloaded = True
